...
class GPU:
    ...
    def execute(self, layer, request_batch=None, block_id=None):
        ...
        if layer.type in [LayerType.FC, LayerType.MATMUL, LayerType.NORM, LayerType.ACT]:
            execute_time = flops/min(operation_intensity*self.peak_memory_bandwidth, self.peak_flops)
            ...
            gpu_onchip_mem_energy = (in1+in2+out)*(self.energy_table['reg'] + self.energy_table['l1'] + self.energy_table['l2'])
            ...
            for kv_head_id in range(layer.n_kv_head):
                for request_id in layer.request_batch.request:
                    ...
                    req_gpu_onchip_mem_energy = (in1+in2+out)*(self.energy_table['reg'] + self.energy_table['l1'] + self.energy_table['l2'])
                    req_energy = (req_gpu_mem_energy + req_gpu_alu_energy + req_gpu_onchip_mem_energy) *  self.num_gpu
                    head_latency[kv_head_id] += req_execute_time
                    head_energy[kv_head_id] += req_energy

            execute_time = max(head_latency)
            ...
            execute_time = 2*max(head_latency)
...
class PIM:
    ...
    def execute(self, layer, request_batch=None, pim_profile_table:PIM_Profile_Table=None, hbf_track_table:HBF_Track_Table=None, GPU=None, block_id=0):
        ...
        if layer.type == LayerType.SpAt_Score_Context:
            ...
            for kv_head_id, request_clusters in activated_clusters_table.items():
                ...
                for request_id, clusters in request_clusters.items():
                    for cluster_id in clusters:
                        ...
                        if key in cluster_mapping_table:
            ...
            if missing_clusters_counts > 0:
                ...
                for kv_head_id in range(layer.n_kv_head):
                    ...
                    for request_id, clusters in missing_clusters[kv_head_id].items():
                        ...
                        gpu_onchip_mem_energy = (in1+in2+out)*(GPU.energy_table['reg'] + GPU.energy_table['l1'] + GPU.energy_table['l2'])
                        gpu_energy += (gpu_mem_energy + gpu_alu_energy + gpu_onchip_mem_energy) *  GPU.num_gpu
                
            # PIM execution has something wrong
            ...
            execute_time = 2 * max(pim_execute_time, gpu_execute_time)
