...
class MoE_Activation_Stat:
    ...
    def __init__(self, stage, n_input_tokens, n_experts=1, experts_per_token=1, zipf_s=1.5):
        self.stage = stage
        self.n_input_tokens = n_input_tokens
        self.n_experts = n_experts
        self.experts_per_token = experts_per_token
        self.expert_token_count = {}  # 存储每个专家被激活的token数量 {expert_id: token_count}
        self.zipf_s = zipf_s  # Zipf 分布参数，s 越大，冷热区分越明显
        
    def get_activated_experts(self, seed=None):
        """
        计算每个token基于冷热区分激活experts_per_token个专家后，返回每个激活专家及其被激活的token数量
        ...
        # 生成专家选择权重（Zipf 分布）：ID 小的专家权重更大（更热门）
        expert_indices = np.arange(1, self.n_experts + 1)  # 从1开始，因为Zipf需要正整数
        weights = expert_indices ** (-self.zipf_s)  # Zipf 分布权重
        weights = weights / weights.sum()  # 归一化
        
        # 为每个token基于权重选择experts_per_token个专家（不放回采样）
        for token_id in range(self.n_input_tokens):
            # 加权不放回采样：每次选择一个专家后，从候选池中移除并重新归一化权重
            selected_experts = []
            remaining_experts = list(range(self.n_experts))
            ...
            for _ in range(self.experts_per_token):
                ...
                # 基于权重选择一个专家
                selected_idx = np.random.choice(len(remaining_experts), p=remaining_weights)
                selected_expert = remaining_experts.pop(selected_idx)
...
class Transformer:
    ...
    def build(self):
        ...
        if self.device == DeviceType.GPU:
            decoder_blocks.append(Attention_Layer('decoder', 'spat_similarity', LayerType.SpAt_Similarity, None, self.tensor_parallel, \
                self.n_q_head, self.n_kv_head, self.dhead, self.cluster_size, self.request_batch))
            decoder_blocks.append(Attention_Layer('decoder', 'spat_score_context', LayerType.SpAt_Score_Context, None, self.tensor_parallel, \
                self.n_q_head, self.n_kv_head, self.dhead, self.cluster_size, self.request_batch))  
            decoder_blocks.append(Attention_Layer('decoder', 'spat_softmax', LayerType.SpAt_Softmax, None, self.tensor_parallel, \
                self.n_q_head, self.n_kv_head, self.dhead, self.cluster_size, self.request_batch))
        ...
        if self.moe_enable:
            #MoE Router
            decoder_blocks.append(FC_Layer('decoder', 'moe', LayerType.FC, "tensor", 1, batch, self.dim, self.n_experts))
            ...
            decoder_blocks.append(FC_Layer('decoder', 'ffn', LayerType.FC, "tensor_col", self.tensor_parallel, batch, self.dim, self.hdim))
            decoder_blocks.append(FC_Layer('decoder', 'ffn', LayerType.MATMUL, "tensor_col", self.tensor_parallel, batch, self.hdim, 1))
            decoder_blocks.append(FC_Layer('decoder', 'ffn', LayerType.FC, "tensor_row", self.tensor_parallel, batch, self.hdim, self.dim))
