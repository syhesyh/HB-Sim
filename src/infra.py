...
class System:
    ...
    def sim(self, max_iteration = None, dynamic_enable = False, scheduling_enable = False):
        ...
        while True:
            ...
            ## fast simulation
                if self.modelinfos["name"] == "llama4":
                    ...
                    for layer in self.moe_model:
                        ...
                        #     if warmup_finish == 1:
                        #         energy, latency, mem_energy, compute_energy = self.GPU.execute(layer)
            ...
            n_iteration += 1
            print(f"iteration {n_iteration}, iteration_energy: {iteration_energy}, iteration_mem_energy: {iteration_mem_energy}, iteration_latency: {iteration_latency}")
            self.request_batch.update()
            if self.device != DeviceType.GPU:
